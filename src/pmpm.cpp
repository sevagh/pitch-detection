...
pitch_alloc::pmpm(const std::vector<T> &audio_buffer, int sample_rate,
    pitch_alloc::Mpm<T> *ma)
{
	...
	for (int n = 0; n < pmpm_consts::N_Cutoffs; ++n) {
		...
		for (int i : max_positions) {
			...
			if (ma->out_real[i] > pmpm_consts::Small_Cutoff<T>) {
...
pitch::pmpm(const std::vector<T> &audio_buffer, int sample_rate)
{
	...
	return pitch_alloc::pmpm(audio_buffer, sample_rate, &ma);
}
...
template std::vector<std::pair<double, double>>
pitch_alloc::pmpm<double>(const std::vector<double> &audio_buffer,
...
template std::vector<std::pair<float, float>>
pitch_alloc::pmpm<float>(const std::vector<float> &audio_buffer,
    int sample_rate, pitch_alloc::Mpm<float> *ma);
